...
public class SwingAnimatorBuilder implements AnimatorBuilder {
  ...
  private static class MyPainter implements SwingAnimatorPainter {
    ...
    //private List<Element<LightController>> _lightElements;
    MyPainter() {
      _roadElements = new ArrayList<Element<Road>>();
      _lightElements = new ArrayList<Element<TrafficLight>>();
      //_lightElements = new ArrayList<Element<LightController>>();
      
    }    
    /*void addLight(LightController x, Translator t) {
        _lightElements.add(new Element<LightController>(x,t));
      }*/
    ...
    public void paint(Graphics g) {
      ...
      for (Element<TrafficLight> e : _lightElements) {
    	//for (Element<LightController> e : _lightElements) {
        //if (e.x.getState()) {
    	  if(e.x.getCurrentState() == e.x.getGreenState()){
      ...
      for (Element<Road> e : _roadElements) {
        ...
        //for (Car d : e.x.getCars().toArray(new Car[0])) {
          ...
          XGraphics.fillOval(g, e.t, d.getFrontPosition(), 0, MP.carLength, VP.elementWidth);
        }
      }
    }
  }
